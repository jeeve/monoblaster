...
export function tryToGoLeft(decor, players, player, setPlayer) {
  ...
  function getSpritesArroundPlayer() {
    ...
    return [Util.getIndex(i-1, j-1), Util.getIndex(i-1, j), Util.getIndex(i-1, j+1)]
  }

  function getBlocksNear() {
    return getSpritesArroundPlayer().filter((sprite) => {
      if (decor[sprite].image !== "") {
        return decor[sprite].x + 32 > player.x - Init.dx && decor[sprite].x + 32 <= player.x;
      }
      return false;
    });
  }
  ...
  getBlocksNear().forEach(sprite => {  
     if ((decor[sprite].y + 32 > player.y && decor[sprite].y + 32 < player.y + 32) || (decor[sprite].y < player.y + 32 && decor[sprite].y > player.y)) {
       ok = false;
